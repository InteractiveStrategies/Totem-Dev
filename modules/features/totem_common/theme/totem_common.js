...
(function($) {
  ...
  Drupal.behaviors.totem_common = {
    attach : function(context, settings) {
      ...
      // Workaround for AJAX forms containing an AJAX-enabled element getting
      // stuck as "ajaxing".
      // @see https://drupal.org/node/2102349
      Drupal.behaviors.totem_common.ajaxFormAjaxingWorkaround(context, settings);
    },
    ...
    modalAutoload : function(context, settings) {
      ...
      // First check whether settings.variables exists. On AJAX requests, the
      // settings passed in the response take precedence if they exist. In such
      // cases (when we're not rendering a full page), settings.variables are
      // not passed.
      // @see Drupal.ajax.prototype.commands.insert()
    ...
    modalFormBtnSubmitting: function(context, settings) {
      ...
      // Mark modal form submit buttons during processing to show the user that
      // something is happening.
      // If Clientside Validation module is not enabled for this form and errors
      // are found via regular AJAX POST, the form will be rebuilt which resets
      // these changes.
      ...
      // If Clientside Validation is enabled for this form, luckily it triggers
      // a custom jQuery event when errors are found. Oddly, it directly calls
      // jQuery.event.trigger() which triggers the event but not on any specific
      // DOM element(s), I think?
      // @see Drupal.clientsideValidation.prototype.bindForms()
      $('form.ctools-use-modal-processed', context).once('totem-common-modal-form-error', function() {
          $(this).bind('clientsideValidationFormHasErrors', function(event, formCurrentTarget) {
            // The arg passed to the trigger() call - formCurrentTarget here -
            // doesn't seem to be either the form itself or even related to the
            // form element(s) currently in error, so we don't rely on it.
            // Also, this event is triggered multiple times on a single
            // attempted submit, even if only one form element is in error.
            var $form = $(this);
            // Confirm that Clientside Validation has found errors on this form.
            if ($form.find('.form-item input.error').length) {
              // Look for a submit button in our custom "in-progress" state.
              var $btn = $form.find('input.modal-form-submitting');
              if ($btn.length) {
                $btn.attr('value', $btn.data('valueOriginal'));
                $btn.removeClass('modal-form-submitting');
              }
            }
          });
      });
    ...
    ajaxFormAjaxingWorkaround: function(context, settings) {
      // Workaround for modal forms containing an AJAX-enabled element (e.g. a
      // file upload field). When the element's AJAX request occurs, the
      // Drupal.ajax object for the associated form gets stuck having
      // ajaxing = true. This blocks submitting the entire form thereafter.
      // @see Drupal.ajax.prototype.eventResponse(), Drupal.ajax() --
      // ajax.options.beforeSubmit(), ajax.options.complete(),
      // https://drupal.org/node/2102349
      $.each(Drupal.ajax, function(ajaxId, ajax) {
        if (ajax._ajaxingFixed == undefined) {
          // Replace with our version, "patched" as noted.
          ajax.options.complete = function (response, status) {
            ajax.ajaxing = false;

            // PATCH -----------------------------------------------------------
            // If this element is part of an AJAX form, we also denote the form
            // as no longer "ajaxing".
            if (ajax.form) {
              Drupal.ajax[ajax.form.attr('id')].ajaxing = false;
            }
            // END PATCH -------------------------------------------------------
          ...
          // Mark the ones we've done... since this needs to happen on every
          // attachBehaviors() in case of newly created ajax-ified elements,
          // but no need to repeat for existing ones.
          ajax._ajaxingFixed = true;
        }
      });
    }
  }
